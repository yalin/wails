...
EnableDefaultContextMenu enables the browser's default context-menu in production (Without the devtools inspector). This menu is already enabled in development, as well as in debug builds and production builds with the `-devtools` flag.
